...
export default function ListingItem({ listing, id, onEdit, onDelete }) {
    ...
    rounded-md overflow-hidden transition-shadow duration-150 m-[10px] ">
        <Link className="contents" to={`/category/${listing.type}/${id}`}>
            <img  className="h-[170] w-full object-cover hover:scale-105
            ...
            <div className="w-full p-[10px] flex-1">
                ...
                <p className="text-[#457b9d] mt-2 font-semibold">
                    Ksh {listing.offer ? listing.discountedPrice 
                        .toString()
                        .replace(/\B(?=(\d{3})+(?!\d))/g, ",")
                ...
                <div className="flex items-center mt-[10px] 
                    ...
                    <div className="flex items-center space-x-l">
                        <p className="font-bold text-xs">{listing.bathrooms > 1 ? `${listing.bathrooms} Bathrooms`
                            : "1 Bathroom"}</p>
                    </div>
                </div>
            </div>
        ...
        {onEdit && (
            ...
            h-4 cursor-pointer text-black-500" 
